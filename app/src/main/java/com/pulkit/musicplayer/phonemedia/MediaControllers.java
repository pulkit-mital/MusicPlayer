...
import com.pulkit.musicplayer.data.FavoriteTableHelper;
import com.pulkit.musicplayer.data.MostRecentTableHelper;
...
public class MediaControllers {
    ...
    private static volatile MediaControllers Instance = null;

    public static enum SonLoadFor {
        All, Gener, Artis, Album, Musicintent, MostPlay, Favorite, ResecntPlay
    }

    public static MediaControllers getInstance() {
        MediaControllers localInstance = Instance;
        if (localInstance == null) {
            synchronized (MediaController.class) {
                localInstance = Instance;
                if (localInstance == null) {
                    Instance = localInstance = new MediaControllers();
                }
            }
        }
...
//                sortOrder = MediaStore.Audio.Media.DATE_ADDED + " desc";
            ...
            case MostPlay:
                cursor = MostRecentTableHelper.getInstance(context).getMostPlay();
                songsList = getSongsFromSQLDBCursor(cursor);
                break;

            case Favorite:
                cursor = FavoriteTableHelper.getInstance(context).getFavoriteSongList();
                songsList = getSongsFromSQLDBCursor(cursor);
                break;
        }
        return songsList;
    ...
    private ArrayList<SongDetail> getSongsFromSQLDBCursor(Cursor cursor) {
        ...
        try {
            if (cursor != null && cursor.getCount() >= 1) {
                ...
                while (cursor.moveToNext()) {
                    long ID = cursor.getLong(cursor.getColumnIndex(FavoriteTableHelper.ID));
                    long album_id = cursor.getLong(cursor.getColumnIndex(FavoriteTableHelper.ALBUM_ID));
                    String ARTIST = cursor.getString(cursor.getColumnIndex(FavoriteTableHelper.ARTIST));
                    String TITLE = cursor.getString(cursor.getColumnIndex(FavoriteTableHelper.TITLE));
                    String DISPLAY_NAME = cursor.getString(cursor.getColumnIndex(FavoriteTableHelper.DISPLAY_NAME));
                    String DURATION = cursor.getString(cursor.getColumnIndex(FavoriteTableHelper.DURATION));
                    String Path = cursor.getString(cursor.getColumnIndex(FavoriteTableHelper.PATH));
    ...
    public static void setPhonemediacontrolinterface(PhoneMediaControlINterface phonemediacontrolinterface) {
        MediaControllers.phonemediacontrolinterface = phonemediacontrolinterface;
    }
