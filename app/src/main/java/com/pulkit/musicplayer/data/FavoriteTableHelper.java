...
public class FavoriteTableHelper {
    ...
    public static final String TABLE_NAME = "ResentPlay";
    ...
    private static FavoriteTableHelper mInstance;
    private SQLiteDatabase sampleDB;


    public static synchronized FavoriteTableHelper getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new FavoriteTableHelper(context);
        }
        return mInstance;
    }
    ...
    public FavoriteTableHelper(Context context_) {
    ...
    public Cursor getFavoriteSongList() {
        Cursor mCursor = null;
        try {
            String sqlQuery = "Select * from " + TABLE_NAME + " where " + ISFAVORITE + "=1";
            sampleDB = dbHelper.getDB();
    ...
    public boolean getIsFavorite(SongDetail mDetail) {
        Cursor mCursor = null;
        try {
            String sqlQuery = "Select * from " + TABLE_NAME + " where " + ID + "=" + mDetail.getId() + " and " + ISFAVORITE + "=1";
            sampleDB = dbHelper.getDB();
            mCursor = sampleDB.rawQuery(sqlQuery, null);
