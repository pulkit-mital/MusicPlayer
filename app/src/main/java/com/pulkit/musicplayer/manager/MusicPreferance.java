...
public class MusicPreferance {
    ...
    public static ArrayList<SongDetail> getPlaylist(Context context) {
        if (playlist == null || playlist.isEmpty()) {
            ...
            playlist = MediaControllers.getInstance().getList(context, id, MediaControllers.SonLoadFor.values()[type], path);
        }
        return playlist;
    }

    public static ArrayList<SongDetail> getPlaylist(Context context, String path) {
        MediaController.getInstance().type = MediaControllers.SonLoadFor.Musicintent.ordinal();
        MediaController.getInstance().id = -1;
        MediaController.getInstance().currentPlaylistNum = 0;
        MediaController.getInstance().path = path;
        playlist = MediaControllers.getInstance().getList(context, -1, MediaControllers.SonLoadFor.Musicintent, path);
        if (playlist != null && !playlist.isEmpty())
