...
package com.pulkit.musicplayer.manager;
...
public class MusicPlayerService extends Service implements AudioManager.OnAudioFocusChangeListener, NotificationManager.NotificationCenterDelegate {
    ...
    private void createNotification(SongDetail mSongDetail) {
        try {
            ...
            RemoteViews simpleContentView = new RemoteViews(getApplicationContext().getPackageName(), R.layout.layout_player_small_notification);
            RemoteViews expandedView = null;
            if (supportBigNotifications) {
                expandedView = new RemoteViews(getApplicationContext().getPackageName(), R.layout.layout_player_big_notification);
            }

            Intent intent = new Intent(App.applicationContext, MusicPlayerBaseActivity.class);
            intent.setAction("openplayer");
            intent.setFlags(32768);
            PendingIntent contentIntent = PendingIntent.getActivity(App.applicationContext, 0, intent, 0);
            ...
            Bitmap albumArt = audioInfo != null ? audioInfo.getSmallCover(App.applicationContext) : null;
...
//                notification.bigContentView.setTextViewText(R.id.player_albumname, albumName);
            ...
            if (remoteControlClient != null) {
                ...
                if (audioInfo != null && audioInfo.getCover(App.applicationContext) != null) {
                    metadataEditor.putBitmap(RemoteControlClient.MetadataEditor.BITMAP_KEY_ARTWORK,
                            audioInfo.getCover(App.applicationContext));
                }
